...
def fahrenheit_to_celsius(temp_f: float) -> float:
    from scipy.constants import convert_temperature
    return convert_temperature(temp_f, 'F', 'C')

def inch_to_millimeter(inch: float) -> float:
    return inch*25.4


def create_udfs(session):
    session.use_schema('ANALYTICS')
    _ = session.sql('CREATE OR REPLACE STAGE conversion_udfs').collect()
    f_to_c_udf = session.udf.register(fahrenheit_to_celsius,
                                    name='FAHRENHEIT_TO_CELSIUS',
                                    is_permanent=True,
                                    replace=True,
                                    stage_location='@conversion_udfs',
                                    packages=['scipy'])
    i_to_m_udf = session.udf.register(inch_to_millimeter,
                                        name='INCH_TO_MILLIMETER',
                                        is_permanent=True,
                                        replace=True,
                                        stage_location='@conversion_udfs',
                                        packages=[],
...
def create_daily_city_metrics_table(session):
    dcm = session.create_dataframe([[None]*len(DAILY_CITY_METRICS_SCHEMA.names)], schema=DAILY_CITY_METRICS_SCHEMA) \
                        .na.drop() \
                        .write.mode('overwrite').save_as_table('DAILY_CITY_METRICS')
    dcm = session.table('DAILY_CITY_METRICS')


def update_daily_city_metrics(session):

    _ = session.sql('ALTER WAREHOUSE HOL_WH SET WAREHOUSE_SIZE = XLARGE').collect()

    print("{} records in stream".format(session.table('ORDERS_STREAM').count()))
    orders_stream_dates = session.table('ORDERS_STREAM').select(F.col("ORDER_TS_DATE").alias("DATE")).distinct()
    # orders_stream_dates.limit(5).show()

    orders = session.table("ORDERS_STREAM").group_by(F.col('ORDER_TS_DATE'), F.col('PRIMARY_CITY'), F.col('COUNTRY')) \
    ...
    weather_agg = weather.group_by(F.col('DATE_VALID_STD'), F.col('CITY_NAME'), F.col('COUNTRY_C')) \
                        .agg( \
                            ...
                            F.avg(F.call_udf("ANALYTICS.FAHRENHEIT_TO_CELSIUS", F.col("AVG_TEMPERATURE_AIR_2M_F"))).alias("AVG_TEMPERATURE_C"), \
                            F.avg("TOT_PRECIPiTATION_IN").alias("AVG_PRECIPITATION_IN"), \
                            F.avg(F.call_udf("ANALYTICS.INCH_TO_MILLIMETER", F.col("TOT_PRECIPiTATION_IN"))).alias("AVG_PRECIPITATION_MM"), \
